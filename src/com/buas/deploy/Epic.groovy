 ...
 */
class Epic implements Serializable {
    def steps
    ...
    def deploy(Map config) {
        ...
        def buildVersion = config.buildVersion ?: "${steps.env.JOB_BASE_NAME}-${steps.env.BUILD_NUMBER}"

        def platformDir = steps.utilWin.platformOutputDir(platform)
        def buildRoot = "${contentDir}\\${platformDir}"

        steps.withCredentials([steps.string(credentialsId: clientSecretId, variable: 'EPIC_SECRET')]) {
            steps.bat(label: "Deploy to Epic Games Store",
                script: "\"${bptPath}\" " +
                        "-mode=UploadBinary " +
